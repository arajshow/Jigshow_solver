...
const initNumberInputEvent = () => {
    console.log("initNumberInputEvent");
    ...
    // to work on it for getting data directly by clicking keyboard

    cells[selected_cell].addEventListener("keydown", (e)=>{
        const input_by_key = e.key;
        console.log("key pressed", input_by_key);

        if(typeof(input_by_key)==Number && input_by_key>0 && input_by_key<10){

            if (!cells[selected_cell].classList.contains('filled')) {
                cells[selected_cell].innerHTML = input_by_key;
                cells[selected_cell].setAttribute('data-value', input_by_key);
                // add to answer
                let row = Math.floor(selected_cell / CONSTANT.GRID_SIZE);
                let col = selected_cell % CONSTANT.GRID_SIZE;
                su_answer[row][col] = input_by_key;
                // save game
                saveGameInfo()
                // -----
                removeErr();
                checkErr(input_by_key);
                cells[selected_cell].classList.add('zoom-in');
                setTimeout(() => {
                    cells[selected_cell].classList.remove('zoom-in');
                }, 500);
